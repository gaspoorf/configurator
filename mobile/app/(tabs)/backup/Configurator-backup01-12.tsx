...
function Model({ goToStep, plane, onPlaneChange }: { goToStep: (stepIndex: number) => void; plane: number; onPlaneChange: (value: number) => void; }) {
    // const asset = Asset.fromModule(require("../../assets/3d/octahedre.glb"));
    const asset = Asset.fromModule(require("../../../assets/3d/keypad-slider.glb"));
    ...
    const handlePointerMove = (e: ThreeEvent<PointerEvent>) => {
        ...
        if (e.ray.intersectPlane(planeRef.current, intersectionPoint.current)) {
            ...
            const mappedValue = THREE.MathUtils.mapLinear(newX, minX, maxX, 0, 10);

            onPlaneChange(mappedValue);
        }
    };
    ...
    return (
        ...
        <primitive
            ...
            scale={8}
        />
        <GyroModelController groupRef={groupRef} />
        </group>
    );
}
...
export default function App({ userName }: Props) {
    ...
    const renderStep = () => {
        ...
        case 0:
            ...
            <View style={styles.sliderContainer}>
                <Text style={styles.label}>A quelle fréquence {userName} prend l’avion ? {Math.round(plane)}</Text>
                <Slider
                style={styles.slider}
                minimumValue={0}
                maximumValue={10}
        ...
        case 1:
            return (
            <View style={styles.sliderContainer}>
                <Text>Quel moyen de transport utilise {userName}:</Text>
        ...
        case 2:
            return (
            <View style={styles.sliderContainer}>
                <Text style={styles.label}>A quelle fréquence {userName} utilise l'IA ? {Math.round(promptIA)}</Text>
                <Slider
                    style={styles.slider}
                    minimumValue={0}
                    maximumValue={300}
        ...
        case 3:
            return (
            <View style={styles.sliderContainer}>
                <Text style={styles.label}>A quelle fréquence {userName} mange de la viande ? {Math.round(meat)}</Text>
                <Slider
                    style={styles.slider}
                    minimumValue={0}
                    maximumValue={18}
        ...
        default:
            return (
                <Text style={styles.doneText}>Voici maintenant votre map</Text>
            );
        }
    };
    ...
    function next() {
        setIndex(i => (i + 1) % cameraPositions.length)
    }
    ...
    //                 <CameraController index={index} />
    //                 <Suspense fallback={null}>
    //                     <Model goToStep={goToStep} plane={plane} onPlaneChange={handlePlaneChange}/>
    //                 </Suspense>
    //             </Canvas>
    //         </View> 
        ...
        <View style={styles.container}>
            ...
            <View style={styles.canvas}>
                ...
                >
                 ...
                 <Button title="Next View" onPress={next} />
            ...
            <View style={styles.buttons}>
                ...
                {currentStep > 0 && (
                    <TouchableOpacity onPress={() => { prev(); prevStep(); }} style={styles.button}>
                        <Text style={styles.buttonText}>←</Text>
                    </TouchableOpacity>
                )}

                {currentStep < steps.length - 1 && (
                    <TouchableOpacity onPress={() => { next(); nextStep(); }} style={styles.button}>
                        <Text style={styles.buttonText}>→</Text>
                    </TouchableOpacity>
                )}

                {currentStep === steps.length - 1 && (
                    <TouchableOpacity onPress={() => { next(); nextStep(); }} style={[styles.button, styles.buttonFinish]}>
